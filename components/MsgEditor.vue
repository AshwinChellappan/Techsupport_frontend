...
onMounted(()=>{
emitter.on('selectedValue', (newValue) => { 
...
const send = () => {
  ...
  if (msg.length > 0) {
    ...
    const input={'role':'user','content':msg};
    const messageList=[input];
    console.log(messageList)
    //props.sendMessage({content: msg, tool: item.name, message_type: item.type})
    props.sendMessage(messageList)
  }
  message.value = ""
...
.v-input__control .v-field__outline__notch {
