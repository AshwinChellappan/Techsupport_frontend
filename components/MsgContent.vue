...
watchEffect(async () => {
  ...
  let messageValue = contentHtml.value;
  const regex = /<a\s+(?:[^>]*?\s+)?href=(["'])(.*?)\1/g;
  const newMessageValue = messageValue.replace(regex, function(match, p1, p2) {
    return '<a target="_blank" href=' + p1 + p2 + p1;
  });
  contentHtml.value = newMessageValue;
  await nextTick()
...
.chat-msg-content { 
    background: rgb(239,250,245) !important;
    color: rgb(37,121,83) !important;
}
.chat-msg-content a {
  color: rgb(9, 76, 138);
}
.v-btn.v-theme--light.text-primary {
  color: rgb(37,121,83) !important;
}

.v-app-bar.v-toolbar  {
  background: rgb(230, 235, 233) !important;
}
v-navigation-drawer {
  background: rgb(230, 235, 233) !important;
}

.v-btn--icon.bg-primary {
  background: rgb(239,250,245) !important;
  color: rgb(37,121,83) !important;
}
.text-primary {
